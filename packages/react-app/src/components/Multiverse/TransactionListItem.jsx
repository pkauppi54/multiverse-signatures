...
export default function TransactionListItem({
  ...
  const [txnData, setTxnData] = useState({}); // All of the transactions in the wallet, for a single one use txnData[item.hash]
    ...
    if (toSelf && txnData[item.hash]?.functionFragment?.name == "addSigner") {
      return (
        <>
          ...
          <span style={{ fontSize: 16 }}>New min. signatures: {txnData[item.hash]?.args[1]?.toNumber()}</span>
          <>{children}</>
        </>
      );
      ...
      return (
        <>
          <div>
            <i> Unknown transaction type... If you are reading this please screenshot and sen to @Stealthyzzzz </i>
          </div>
          ...
          >
            ...
            <b>To :&nbsp;</b>
            <Address address={item.to} ensProvider={mainnetProvider} blockExplorer={blockExplorer} price={price} />
          </div>
        </>
      );
    }
